...
/** Sign In Button for step 4 when not signed in */
  ...
  return (
    <>
      ...
      >
        ...
        {/* Step 4 - Different options based on sign-in status */}
        {step === 4 && (
          <div className="space-y-2">
            {/* Show payment summary only if signed in */}
            {isSignedIn && (
              <PaymentSummary onOpenWalletModal={handleOpenWalletModal} />
            )}

            {/* Show appropriate button based on sign-in status */}
            {isSignedIn ? (
              <ConfirmButton
                isDepartureFlow={isDepartureFlow}
                charging={charging}
              ...
              <SignInButton onClick={handleOpenSignIn} />
            )}
          </div>
        )}

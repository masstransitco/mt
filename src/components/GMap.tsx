...
// Dynamically load GaussianSplatModal for performance
...
export default function GMap({ googleApiKey }: GMapProps) {
  ...
  // The user’s “main” route from departure→arrival
  const route = useAppSelector(selectRoute);

  // The dispatch→departure route
  ...
  /**
   * handleStationClick
   * Called when user clicks on a station marker or 3D station cube
   */
  const handleStationClick = useCallback(
    (station: StationFeature) => {
      if (bookingStep === 1 || bookingStep === 2) {
        // If we’re still picking or confirming departure
        ...
        // If we’re picking or confirming arrival
      ...
      // Open the station detail sheet
      setDetailKey((prev) => prev + 1);
      setForceSheetOpen(true);
      setOpenSheet("detail");
      setPreviousSheet("none");

      // Un-minimize the bottom sheet if needed
      if (isSheetMinimized) {
  ...
  // ---------------
  // Hide spinner once map & data are loaded
  // ---------------
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const intervalId = setInterval(() => {
      ...
      if (renderer && camera) {
        ...
        // Track mouse move to handle station hover, etc.
        ...
        // Click => check if we clicked a 3D station
  ...
  // ---------------
  // 1) Auto-fetch user's route (departure→arrival)
  // ---------------
  useEffect(() => {
  ...
  // ---------------
  // 2) Auto-fetch dispatch->departure route at step=2
  // ---------------
  useEffect(() => {
    if (bookingStep === 2 && departureStationId) {
      const departureStation = stations.find((s) => s.id === departureStationId);
      if (departureStation) {
        dispatch(fetchDispatchDirections(departureStation));
      }
    } else {
      // If not step 2 or no departure station, clear it
      dispatch(clearDispatchRoute());
    }
  }, [bookingStep, departureStationId, stations, dispatch]);
  ...
  const handleClearDepartureInSelector = () => {
    ...
    // If user was viewing station detail, hide it
    if (openSheet === "detail") {
      setOpenSheet("none");
      setPreviousSheet("none");
    }
  ...
  // ---------------
  // Determine the station to show in detail
  // ---------------
